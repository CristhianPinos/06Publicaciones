    ...
    public class Autor
    {
        ...
        public static string Eliminar(string idAutor)
        {
            try
            {
                using (var conexion = Conexion.GetConnection())
                {
                    var consultaEliminarDependencias = "DELETE FROM titleauthor WHERE au_id = @IdAutor";
                    using (var comandoEliminarDependencias = new SqlCommand(consultaEliminarDependencias, conexion))
                    {
                        comandoEliminarDependencias.Parameters.AddWithValue("@IdAutor", idAutor);
                        comandoEliminarDependencias.ExecuteNonQuery();
                    }
